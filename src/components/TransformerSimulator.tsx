...
const stepDescriptions: Record<SimulationStep, { title: string; description: string }> = {
    ...
    embedded: { title: "Step 2: Embedding", description: "Each token is converted into a numerical vector (an 'embedding'). This vector represents the token's meaning." },
    positional: { title: "Step 3: Positional Encoding", description: "A positional vector is added to each embedding. This gives the model information about the order of the tokens in the sequence." },
    qkv: { title: "Step 4: Query, Key, Value", description: "For each token, the model creates three vectors: a Query (Q), a Key (K), and a Value (V). These are used to calculate attention scores." },
    attention: { title: "Step 5: Multi-Head Attention", description: "The model calculates attention scores by comparing the Query of one token to the Key of every other token. This determines how much focus to place on other tokens when processing the current one. The scores are then used to create a weighted sum of the Value vectors." },
    ffn: { title: "Step 6: Feed-Forward Network", description: "The output from the attention layer is passed through a simple neural network to perform further processing." },
    output: { title: "Step 7: Prediction", description: "Finally, the output of the transformer block is converted into a probability distribution over the entire vocabulary to predict the next token." },
};
            ...
            <CardContent className="space-y-6">
                ...
                <div className="space-y-4 min-h-[400px]">
                    ...
                    <div className={cn("transition-opacity duration-500", currentStepIndex >= 1 ? 'opacity-100' : 'opacity-0')}>
                        <h4 className="font-semibold text-sm mb-2 flex items-center"><FileInput className="mr-2 h-4 w-4" />1. Tokens</h4>
                    ...
                    {(currentStepIndex >= 2) && (
                         <div className={cn("transition-opacity duration-500", currentStepIndex >= 2 ? 'opacity-100' : 'opacity-0')}>
                            <h4 className="font-semibold text-sm mb-2 flex items-center"><Zap className="mr-2 h-4 w-4" />2. Embeddings</h4>
                            <div className="flex flex-col gap-2 bg-background/50 p-4 rounded-lg border">
                               {tokens.map((token, i) => (
                                   <div key={i} className={cn("flex items-center gap-4 transition-all duration-500", currentStepIndex >= 2 ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4')} style={{transitionDelay: `${i * 100}ms`}}>
                                ...
                                {currentStepIndex === 3 && (
                                     <p className="text-xs text-muted-foreground mt-2 text-center">Token Embedding + Positional Encoding = Final Input Vector</p>
                                )}
                            </div>
                        </div>
                    )}
                    ...
                    {(currentStepIndex >= 4) && (
                         <div className={cn("transition-opacity duration-500", currentStepIndex >= 4 ? 'opacity-100' : 'opacity-0')}>
                            <h4 className="font-semibold text-sm mb-2 flex items-center"><Bot className="mr-2 h-4 w-4" />3. Transformer Block</h4>
                            <div className="flex flex-col gap-4 bg-background/50 p-4 rounded-lg border">
                                <div>
                                    ...
                                    {currentStepIndex === 5 && <p className="text-xs text-muted-foreground mt-2 text-center">For the token 'ai', the model is paying high attention to 'what'.</p>}
                                </div>
                                    ...
                                    <h5 className={cn("font-semibold text-xs text-center mb-2 transition-colors", currentStepIndex >= 6 ? 'text-primary' : 'text-muted-foreground')}>Feed-Forward Network</h5>
                                     <div className="flex justify-center">
                                          <div className="p-4 bg-muted rounded-lg w-full md:w-1/2 flex justify-center items-center">
                                            <p className="text-sm text-muted-foreground">Processed Vectors</p>
                                          </div>
                                     </div>
                                </div>
                            </div>
                    ...
                    {/* Output */}
                     {(currentStepIndex >= 7) && (
                         <div className={cn("transition-opacity duration-500", currentStepIndex >= 7 ? 'opacity-100' : 'opacity-0')}>
                            <h4 className="font-semibold text-sm mb-2 flex items-center"><Zap className="mr-2 h-4 w-4" />4. Prediction</h4>
                            <div className="flex flex-col gap-2 bg-background/50 p-4 rounded-lg border">
                               <p className="font-mono text-sm text-primary">{finalOutput}</p>
                            </div>
                        </div>
                    )}
